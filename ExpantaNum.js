//https://github.com/Naruyoko/ExpantaNum.js
//https://github.com/Naruyoko/ExpantaNum.js
//https://github.com/Naruyoko/ExpantaNum.js
//https://github.com/Naruyoko/ExpantaNum.js
//https://github.com/Naruyoko/ExpantaNum.js
...
//expantanum expansion
var N = ExpantaNum, sub = N.sub, div = N.div, pow = N.pow, neg = N.neg, abs = N.abs, lte = N.lte, lt = N.lt, gte = N.gte, gt = N.gt, random = Math.random, floor = N.floor, ceil = N.ceil, round = N.round, sqrt = N.sqrt
var eq = N.eq
function intRandom(a,b) {
	return floor(add(mul(random(), add(1,sub(b,a)) ),a))
}
function normalRandom() {
	var u = random()*2-1, v = random()*2-1
	while (u*u + v*v >= 1) {
		u = random()*2-1, v = random()*2-1
	}
	s = Math.sqrt(u*u+v*v)
	return u*Math.sqrt(-2*Math.log(s)/s)
}
function chanceRandom(p) {
	if (gt(p,1e16)) return false //just no
	p = div(1,p).toString()*1
	//inverse probability
	return regularChanceRandom(p)
}
function regularChanceRandom(p) {
	//normal probability
	return random() < p
}
function add() {
	var A = [...arguments], R = N(0)
	for (let k in A) R = N.add(R,A[k])
	return R
}
function mul() {
	var A = [...arguments], R = N(1)
	for (let k in A) R = N.mul(R,A[k])
	return R
}
function roll(n,p) {
	n = N(n), p = N(p)
	//p is inverse prob btw
	if (lte(n,100)) {
		n = n.toNumber()
		p = div(1, p).toString()*1
		var r = N(0)
		while (n--) if (regularChanceRandom(p)) r = add(r, 1)
		return r
	}

	//better calculateion
	var mean = div(n,p) // n/p
	if (gt(mean,"1e40")) return mean

	var V = sqrt(mul(mean,sub(1,div(1, p))))
	var amount = add(mul(V,normalRandom()),mean) // V*nr() + mean

	if (lt(amount, 0)) return 0 //negative amount
	if (gt(amount,1000)) return floor(amount) //a lot

	var excess = amount.mod(1), amount = floor(amount) //excess
	if (chanceRandom(div(1, excess))) amount = add(amount, 1)
	return amount
	//HOLY FUCKING SHIT
